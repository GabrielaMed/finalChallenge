...
describe("Get reserve by id", () => {
  ...
  beforeAll(async () => {
    ...
    getReserveByIdService = new GetReserveByIdService(reserveRepository);
  ...
  it("should be able to get a reserve by id", async () => {
    ...
    const reserveExists = await getReserveByIdService.execute(String(id));

    expect(reserveExists).toHaveProperty("_id");
  });

  it("should not be able to get a reserve with invalid id", async () => {
    const id = "invalid";

    await expect(getReserveByIdService.execute(id)).rejects.toEqual(
      new AppError("Id is different from default!", 400)
    );
  });

  it("should not be able to get an unexisting reserve", async () => {
